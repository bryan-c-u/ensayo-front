...
/**
 * M09 - Planes de suscripción. Lista los planes reales de ink-ms-suscripciones y
 * arranca el cobro: al pulsar "Elegir plan" se crea/renueva la suscripción y se
 * redirige al checkout de Mercado Pago (o, si el plan es gratuito, queda activo).
 */
@Component({
    ...
    peticion$.subscribe({
      next: (resp) => {
        ...
        if (!resp.referenciaTransaccion) {
          // Plan gratuito: iniciarPago ya activó la suscripción, no hay nada que cobrar.
          this.router.navigate(['/subscriptions/estado']);
          return;
        }
        ...
        this.router.navigate(['/subscriptions/pago', resp.referenciaTransaccion], {
